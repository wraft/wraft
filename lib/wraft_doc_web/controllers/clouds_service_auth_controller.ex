defmodule WraftDocWeb.CloudServiceAuthController do
  @moduledoc """
  ...
  alias WraftDoc.CloudService.CloudAuth
  require Logger
  ...
  def google_callback(conn, %{"code" => code} = params) do
    ...
    with {:ok, token_data} <- CloudAuth.google_drive_token(code),
         {:ok, _token} <- AuthTokens.save_cloud_service_token(user, token_data, :google_drive) do
  ...
  def dropbox_callback(conn, %{"code" => code} = params) do
    ...
    with {:ok, token_data} <- CloudAuth.dropbox_token(code),
         {:ok, _token} <- AuthTokens.save_cloud_service_token(user, token_data, :dropbox) do
      redirect(conn, to: get_redirect_path(params))
    else
      {:error, reason} ->
  ...
  def onedrive_callback(conn, %{"code" => code} = params) do
    ...
    with {:ok, token_data} <- CloudAuth.onedrive_token(code),
         {:ok, _token} <- AuthTokens.save_cloud_service_token(user, token_data, :onedrive) do
      redirect(conn, to: get_redirect_path(params))
    else
      {:error, reason} ->
