defmodule WraftDocWeb.CloudServiceController do
  ...
  alias WraftDoc.CloudService.Clouds
  ...
  def list_gdrive_files(conn, params) do
    with token when not is_nil(token) <- AuthTokens.get_latest_token(:google_oauth),
  ...
  def download_gdrive_file(conn, %{"file_ids" => file_ids}) do
    ...
    with token when not is_nil(token) <- AuthTokens.get_latest_token(:google_oauth) do
  ...
  def list_dropbox_files(conn, params) do
    with token when not is_nil(token) <- AuthTokens.get_latest_token(:dropbox),
  ...
  def download_dropbox_file(conn, %{"file_ids" => file_ids}) do
    ...
    with token when not is_nil(token) <- AuthTokens.get_latest_token(:dropbox) do
  ...
  def list_onedrive_files(conn, params) do
    with token when not is_nil(token) <- AuthTokens.get_latest_token(:onedrive),
  ...
  def download_onedrive_file(conn, %{"file_ids" => file_ids}) do
    ...
    with token when not is_nil(token) <- AuthTokens.get_latest_token(:onedrive) do
