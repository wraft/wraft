defmodule WraftDocWeb.Api.V1.InstanceController do
  ...
  @doc """
    Add image to the document instance.
  """
  swagger_path :add_image do
    post("/contents/{id}/image")
    summary("Add image to the document instance")
    description("Api to add image to the document instance")

    parameters do
      id(:path, :string, "Instance id", required: true)
      content(:body, Schema.ref(:AddImageRequest), "Image to be added", required: true)
    end
  ...
  def add_image(conn, %{"id" => id, "image" => image}) do
    ...
    with %Instance{} = instance <- Document.show_instance(id, current_user),
      ...
      # render(conn, "add_image.json", %{info: "Image added successfully"})
    end
  end

  @doc """
    Remove image from the document instance.
  """
  swagger_path :remove_image do
    PhoenixSwagger.Path.delete("/contents/{id}/image")
    summary("Remove image from the document instance")
    description("Api to remove image from the document instance")
  ...
  """
  swagger_path :generate_presigned_url do
    get("/contents/{id}/image/presigned_url")
    summary("Generate presigned url for the image")
    description("Api to generate presigned url for the image")
