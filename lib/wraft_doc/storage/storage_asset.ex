defmodule WraftDoc.Storage.StorageAsset do
  use Ecto.Schema
  ...
  schema "storage_assets" do
    ...
    belongs_to(:storage_item, WraftDoc.Storage.StorageItem)
  ...
  def changeset(storage_asset, attrs) do
    storage_asset
    |> cast(attrs, [:storage_key, :storage_backend, :file_size, :checksum_sha256, :mime_type, :encryption_key_id, :compression_type, :thumbnail_path, :preview_path, :processing_status, :upload_completed_at, :storage_item_id, :creator_id, :organisation_id])
    |> validate_required([:storage_key, :storage_backend, :file_size, :mime_type, :processing_status])
  ...
  def public_changeset(storage_asset, attrs) do
    storage_asset
    |> cast(attrs, [:storage_key, :storage_backend, :file_size, :checksum_sha256, :mime_type, :encryption_key_id, :compression_type, :thumbnail_path, :preview_path, :processing_status, :upload_completed_at, :storage_item_id, :organisation_id])
    |> validate_required([:storage_key, :storage_backend, :file_size, :mime_type, :processing_status])
    |> validate_inclusion(:storage_backend, ["local", "s3", "minio", "gcs"])
    |> validate_inclusion(:processing_status, ["pending", "processing", "completed", "failed"])
    |> validate_number(:file_size, greater_than: 0)
  ...
  def update_changeset(storage_asset, attrs) do
    storage_asset
    |> cast(attrs, [:processing_status, :upload_completed_at, :thumbnail_path, :preview_path, :encryption_key_id, :compression_type])
    |> validate_inclusion(:processing_status, ["pending", "processing", "completed", "failed"])
  end
end
