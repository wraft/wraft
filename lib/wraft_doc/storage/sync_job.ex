defmodule WraftDoc.Storage.SyncJob do
  ...
  def changeset(sync_job, attrs) do
    sync_job
    |> cast(attrs, [:job_type, :sync_source, :status, :started_at, :completed_at, :items_processed, :items_failed, :error_details])
    |> validate_required([:job_type, :sync_source, :status, :started_at, :completed_at, :items_processed, :items_failed])
  end
end
