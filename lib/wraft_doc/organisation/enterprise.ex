defmodule WraftDoc.Enterprise do
  ...
  * params - a map with states key
  ...
  def create_organisation(%User{id: user_id} = user, params) do
    ...
    |> Multi.run(:setup_default_repository, fn _repo, %{organisation_logo: organisation} ->
  ...
  def create_personal_organisation(%User{id: user_id} = user, params) do
    Multi.new()
    |> Multi.insert(
      :organisation,
      user
    ...
    |> Multi.run(:repository, fn _repo, %{organisation: organisation} ->
      create_default_repository(organisation, %{"creator_id" => user_id})
    end)
    |> then(fn multi ->
      if self_hosted?() do
