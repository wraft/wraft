defmodule WraftDoc.Billing do
  ...
  @doc """
  Returns boolean value if the plan is a trailing plan.
  """
  @spec is_trailing_plan?(String.t()) :: boolean()
  def is_trailing_plan?(plan_id) do
    query =
      from(p in Plan,
        where: p.plan_id == ^plan_id and not is_nil(p.trial_period),
        select: count(p.id) > 0
      )
  ...
  def change_plan_preview(
    ...
    provider_subscription_id
    |> PaddleApi.update_subscription_preview(provider_product_id)
    |> case do
      {:ok, response} ->
        {:ok, response}
