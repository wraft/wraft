defmodule WraftDoc.AdminWebhooks.AdminWebhookLog do
  @moduledoc """
  Schema for tracking admin webhook execution logs.
  """
  use WraftDoc.Schema
  ...
  @fields ~w(event url http_method request_headers request_body response_status
            ...
            attempt_number triggered_at webhook_id)a
  ...
  schema "admin_webhook_logs" do
    ...
    belongs_to(:webhook, WraftDoc.AdminWebhooks.AdminWebhook)
  ...
  def changeset(webhook_log, attrs \\ %{}) do
    webhook_log
    |> cast(attrs, @fields)
    |> validate_required([:event, :url, :triggered_at, :webhook_id])
    ...
    |> foreign_key_constraint(:webhook_id, message: "Please enter a valid admin webhook")
  end
