...
% TODO: Remove this , added background image in initial phase.
% $if(letterhead)$
% \usepackage{wallpaper}
% \ULCornerWallPaper{1}{$letterhead$}
% $endif$

% Add the background package and configure it for A4 background on Letter size
$if(letterhead)$
...
\backgroundsetup{
    ...
    opacity=1,
    contents={
        \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{$letterhead$} % Replace with your A4 background image
    },
    position=current page.center % Center the background image
}
$endif$
\usepackage{xcolor}
\usepackage{xstring} % for \StrGobbleLeft
...
$if(body_color)$
  ...
  \pagecolor{BodyColor}
$else$
  \definecolor{BodyColor}{HTML}{FF8FFF}
  \pagecolor{BodyColor}
$endif$
...
% replaces pandoc heading sectioning
\makeatletter
