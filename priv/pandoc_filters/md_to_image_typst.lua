...
local function download_image(url)
  ...
  local temp_path = image_folder .. "temp_" .. name_without_ext

  os.execute("curl -s -L -o " .. temp_path .. " " .. url)
  ...
  local data = file:read(32)  -- Read first 32 bytes for detection
  file:close()

  local ext = detect_file_type(data)
  local final_path = get_storage_path(name_without_ext, ext)
...
function Image(img)
  if not os.execute("test -d " .. image_folder) then
    os.execute("mkdir -p " .. image_folder)
    print("Created image folder: " .. image_folder)
  end
