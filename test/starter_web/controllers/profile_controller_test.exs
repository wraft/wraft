defmodule WraftDocWeb.ProfileControllerTest do
  ...
  test "deletes profile and render profile details", %{conn: conn} do
    profile = insert(:profile)
    ...
    count_before = Profile |> Repo.all() |> length

    conn = delete(conn, Routes.v1_profile_path(conn, :delete, profile.uuid))
    count_after = Profile |> Repo.all() |> length()
